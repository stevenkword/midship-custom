...
function midship_accredit_source( $content ) {
	...
	if( $link ) {
		$content = '<p class="post-byline">' . the_author_posts_link() .' at <a target="_blank" href="' . esc_url($link) . '">' . esc_html($title) . '</a>' . '</p>' . $content; //. ' &middot; ' . the_time(get_option('date_format'))
		return $content;
	}
}
add_filter( 'the_content', 'midship_accredit_source', 20 );
