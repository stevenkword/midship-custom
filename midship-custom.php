...
function midship_accredit_source( $content ) {
	...
	if( $link ) {
		$content = '<p class="post-byline">' . get_the_author_link() .' at <a target="_blank" href="' . esc_url($link) . '">' . esc_html($title) . '</a>' . '</p>' . $content; //. ' &middot; ' . the_time(get_option('date_format'))
		return $content;
	}
}
